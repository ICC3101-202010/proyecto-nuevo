...
namespace Entrega3N2
{
    class MainClass
    {
        ...
        void Save(Dictionary<string, User> userDB, List<Media> mediaDB, List<Person> peopleDB, string name)
        {
        ...
        (Dictionary<string, User>, List<Media>, List<Person>) Load(string name)
        {
            Stream stream = new FileStream(name, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None);
            IFormatter formatter = new BinaryFormatter();
